...
class Im2LaTeXModel(nn.Module):
    ...
    def __init__(self, encoder_out_size, embedding_dim, vocab_size, num_layers, num_heads, feedforward_dim, dropout, max_len, device, vocab_dict = {}):
        ...
        invaling_set = [vocab_dict["{"], vocab_dict["}"], vocab_dict["_"], vocab_dict["\\!"], vocab_dict["~"], vocab_dict["."], vocab_dict["\\cdot"], vocab_dict["\\,"]]
        self.invalid_tensor = torch.tensor(invaling_set, dtype=torch.long, device=device)
    ...
    def predict(self, image):
        ...
        for i in range(1, self.max_len):
            ...
            if i > n and torch.all(torch.isin(outputs[:, i-n:i], self.invalid_tensor)):
